...
Req="https://www.admplus.tk/BotGenPlus"
update () {
	...
	[[ ! -e "bot-db/package.json" ]] && wget -O "bot-db/package.json" ${Req}/package.json &>/dev/null
	[[ ! -e "bot-db/bot.js" ]] && wget -O "bot-db/bot.js" ${Req}/bot.js &>/dev/null
	[[ ! -e "bot-db/shell.sh" ]] && wget -O "bot-db/shell.sh" ${Req}/shell.sh &>/dev/null
	[[ ! -e "confbot" ]] && wget -O "confbot" ${Req}/confbot &>/dev/null
...
mensaje () {
	if [[ $1 = "off" ]]; then
		...
		MENSAJE="Removiendo actualizador..."
	else
