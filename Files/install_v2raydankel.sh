...
web_camouflage() {
    ##请注意 这里和LNMP脚本的默认路径冲突，千万不要在installation了LNMP的环境下使用本脚本，否则后果自负
    rm -rf /home/wwwroot
...
nginx_exist_check() {
    if [[ -f "/etc/nginx/sbin/nginx" ]]; then
        ...
        echo -e "${OK} ${GreenBG} Nginx installed by other packages is detected, and continuing the installation will cause conflicts, please handle the installation${Font}"
        exit 1
    else
        nginx_install
    fi
...
nginx_install() {
    ...
    echo -e "${OK} ${GreenBG} About to start compiling installation jemalloc ${Font}"
    sleep 2
    ...
    judge "jemalloc compilation installation"
    echo '/usr/local/lib' >/etc/ld.so.conf.d/local.conf
    ldconfig

    echo -e "${OK} ${GreenBG} About to start compiling installation Nginx, the process will take a little longer, please be patient ${Font}"
    sleep 4
    ...
    judge "Compile check"
    make -j "${THREAD}" && make install
    judge "Nginx compilation installation"
...
ssl_install() {
    ...
    judge "installation SSL certificate generation script dependency"

    curl https://get.acme.sh | sh
    judge "installation SSL certificate generation script"
}
domain_check() {
    read -rp "Please enter your domain name information (eg: dankelthaher.com.mx):" domain
...
acme() {
    if "$HOME"/.acme.sh/acme.sh --issue -d "${domain}" --standalone -k ec-256 --force --test; then
        echo -e "${OK} ${GreenBG} SSL The certificate test is issued successfully and the official issuance begins ${Font}"
    ...
    if "$HOME"/.acme.sh/acme.sh --issue -d "${domain}" --standalone -k ec-256 --force; then
        echo -e "${OK} ${GreenBG} SSL The certificate is generated successfully ${Font}"
        sleep 2
        mkdir /data
...
enable_process_systemd() {
    ...
    if [[ "$shell_mode" != "h2" ]]; then
        ...
        judge "Set Nginx to start at boot"
    fi
...
basic_information() {
    {
        echo -e "${OK} ${GreenBG} V2ray+ws+tls installation was successful"
...
tls_type() {
    if [[ -f "/etc/nginx/sbin/nginx" ]] && [[ -f "$nginx_conf" ]] && [[ "$shell_mode" == "ws" ]]; then
        ...
        echo "Please note that if you use Quantaumlt X / router / old version of Shadowrocket / V2ray core lower than 4.18.1, please select compatibility mode"
        ...
        echo -e "${Error} ${RedBG} Nginx or configuration file does not exist or the current installation version is h2, please execute the correct installation script${Font}"
    fi
}
show_access_log() {
...
menu() {
    ...
    echo -e "${Green}11.${Font} installation 4 in 1 bbr sharp speed installation script"
